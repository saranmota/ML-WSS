...
class triggers:
    cycleTr = False # hacks do dario

def hydraulicSimulator(timeIntPumpStates,h0,iChart):
    
    # Load model ML

    model = joblib.load('xgb_model_fontinha2.joblib')
    ...
    def tarifario(ti,tf):
        ...
        for i in range(0, len(tarifHora)-1):
            if (ti >= tarifHora[i]) and (ti < tarifHora[i+1]):
                ...
                if tf<=tarifHora[i+1]:
                    tarifF = [tarifCusto[i], 0]
                    #dur = tf - ti 
                    #tariffpriceInc = dur * tarifF
                elif tf >=tarifHora[i+1]:
                    print('Atravessou Tarifas')
                    input()
                    tarifF = [tarifCusto[i], tarifCusto[i+1] ]
    ...
    for i in range(len(timeIntPumpStates['timeInt']) - 1):
        ...
        if timeIntPumpStates['timeInt'][i] == 0:
            ...
            # print('Y_PRED_UN',  y_pred_un)
       

            with open('derivads.csv', 'ab') as x_g:
                np.savetxt(x_g,np.ones((1,len(y_pred_un)))*y_pred_un, delimiter=';')
            with open('time.csv', 'ab') as c:
                np.savetxt(c,np.ones((1,1))*dtime, delimiter=';')
            # Integration
...
def simulation(x,nDutyCycles,timeHorizon):
    ...
    def verificar_nivel_dutycycle(x_sec, tempos, niveis, potencia, estados_bomba, tarifa):
        respostas = []
        tolerance = 0.00001
        
        x_final = sorted(x_sec[:len(x_sec)//2] + [x_sec[i] + dur for i, dur in enumerate(x_sec[len(x_sec)//2:])])
        ...
        for t in x_final:
            ...
            #if t == 0 :
            #    print('Collect level (inicio dia)')
            ...
            # Verificar dutycycle      
            if any(abs(t - value)  < tolerance for value in tempos):
                indice = tempos.index(t)
                nivel = niveis[indice]
                power = potencia[indice]
                state = estados_bomba[indice]
            ...
            if nivel is not None:
                respostas.append((t, nivel, power, state, tarif))

        #Adicionar inicio do dia
        primeiro_nivel = niveis[0]
        primeiro_power = potencia[0]
        primeiro_state = estados_bomba[0]
        primeiro_tarif = tarifa[0]
        respostas.insert(0,(0, primeiro_nivel, primeiro_power, primeiro_state, primeiro_tarif))
        
        # Adicionar último nível após o último tempo
        ultimo_nivel = niveis[-1]
        ultimo_power = potencia[-1]
        ultimo_state = estados_bomba[-1]
        ultimo_tarif = tarifa[-1]
        respostas.append((86400, ultimo_nivel, ultimo_power, ultimo_state, ultimo_tarif))
        ...
        for tupla in respostas:
            ...
            tarif_dc.append(tupla[4])
        return tempos_dc, niveis_dc, potencia_dc, p_state_dc, tarif_dc
    ...
    tarifa = [entry['TariffInc'] for entry in timeInc] # todas as tarifas da key 'Tariff'
    #potencia = [round(c,1) for c in potencia]
    ...
    for i in range(0, len(tempo)):
        if tempo [i] %300 ==0:
            aa = tank_final [i]
            level = np.append(level, aa)

    # Voltar para formato dutycycle
    tempos_dc, niveis_dc, potencia_dc, p_state_dc, tarif_dc = verificar_nivel_dutycycle(x_sec, tempos=tempo, niveis=tank_final, potencia=potencia, estados_bomba=pump_state,tarifa=tarifa )
    return tempos_dc, niveis_dc, potencia_dc, p_state_dc, tarif_dc, tempo, tarifa, tank_final, level
