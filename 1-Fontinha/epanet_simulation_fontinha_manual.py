...
def junction_base_demand():
    ...
    for i in range(0,len(junction_idx)):
        if en.ENgetnodevalue(junction_idx[i], en.EN_BASEDEMAND)>0:
            index.append(i)
            base_demand.append(en.ENgetnodevalue(junction_idx[i], en.EN_BASEDEMAND))
            junction_demand["junction_"+str(i)] = []#en.ENgetnodevalue(junction_idx[i], en.EN_BASEDEMAND)
    return junction_demand,index,base_demand
...
def set_initial_level_tank():
    for m in range(0,len(tank_idx)):
        ...
        lim = 1 # fontinha 1# richmond 0.3 
        min_tank_level = en.ENgetnodevalue(tank_idx[m], en.EN_MINLEVEL)
        max_tank_level = en.ENgetnodevalue(tank_idx[m], en.EN_MAXLEVEL)
        en.ENsetnodevalue(tank_idx[m], en.EN_TANKLEVEL, uniform(min_tank_level, max_tank_level)) # value*uniform(0.75, 0.25) novo nivel será maior ou menor 25% que o outro
                                                        # richmond (del value) uniform(min_tank_level + lim, max_tank_level - lim

def set_initial_demand_value():
    for n in index: 
        value = en.ENgetnodevalue(junction_idx[n], en.EN_BASEDEMAND)
...
def demand_value():
    for n in index:
        ...
        #value2 = en.ENgetnodevalue(junction_idx[n], en.EN_BASEDEMAND)
        # Pattern Calculation
        pattern_idx=int(en.ENgetnodevalue(junction_idx[n],en.EN_PATTERN)) 
        pattern_len=en.ENgetpatternlen(pattern_idx)
        pattern_values=[]
        for i in range(1,pattern_len):
            pattern_values.append(en.ENgetpatternvalue(pattern_idx,i))
        #print('Demand \n',value, 'Base_demand \n', value2 )
...
def restore_demand_value():
    for n in range(0,len(index)):
        en.ENsetnodevalue(junction_idx[index[n]], en.EN_BASEDEMAND, base_demand[n]) 
    """
    function restores the initial demand values for the junction nodes in the water network model. The function 
    sets the demand values for each junction node to their original values stored in the base_demand list, which 
...
demand_hour = []
junction_demand,index,base_demand = junction_base_demand()
...
# alterar os valores
en.ENsettimeparam(0, 86400*2) # Experimentar para 1 mês 86400*30
...
while t_step > 0:
    ...
    #print('inicial:', tanks_hour, ' Final : ' , tanks_final_hour)
    #input()
    #time_step.append(t_step)   
...
#epanet_data.to_csv(r'C:\Users\LENOVO\OneDrive - Universidade de Aveiro\universidade\Mestrado\2º Ano\Tese\Códigos\Test_epanet\Samples\48h_5min_fontinha_manual.csv',index=False)
...
epanet_data = pd.read_csv(r'C:\Users\LENOVO\OneDrive - Universidade de Aveiro\universidade\Mestrado\2º Ano\Tese\Códigos\Test_epanet\Samples\fontinha_test.csv', index_col=False)
...
with plt.style.context('science'):
    ...
    plt.ylabel('Demands [L/s]')
    plt.legend()
    plt.suptitle('24h, Test day', fontsize=16, fontweight="bold")
    plt.savefig(os.path.join(imagens_tese,"test_day.pdf"), dpi=300)
    plt.show()
