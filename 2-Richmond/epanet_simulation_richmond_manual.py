...
def set_initial_level_tank():
    for m in range(0,len(tank_idx)):
        ...
        en.ENsetnodevalue(tank_idx[m], en.EN_TANKLEVEL, uniform(min_tank_level+lim, max_tank_level)) # value*uniform(0.75, 0.25) novo nivel será maior ou menor 25% que o outro
...
""" definir controlos da bomba """

# def control_pump(pump_idx):
#    # count = en.ENgetcount(en.EN_CONTROLCOUNT)
#     a = []
#     b = []
#     time_on = [50, 2000, 7000, 20000, 60000, 80000]
#     time_off = [600, 5000, 15000, 50000, 65000, 86000]

  
#     ii = 1
#     jj = 2
#     for j in range(0, int((count/2))):
#         en.ENsetcontrol(ii, en.EN_TIMER, pump_idx[0], 1.0, 0, time_on[j])  #ii = Nº CONTROLO QUERO MUDAR; ESTOU a mudar em relação ao tempo; indice da bomba; se está liga ou desligada; só tem valor se estivermos a controlar por alguma condição; tempo em que vou ligar
#         en.ENsetcontrol(jj, en.EN_TIMER, pump_idx[0], 0.0, 0, time_off[j])     # Aqui estou a alterar os controlos
        
#         jj += 2
#         ii += 2
#     for i in range(1, count+1):
#         b = np.append(b, en.ENgetcontrol(i))
#     control_final = np.reshape(b, (count,5))
#     print("control_final", control_final)
...
epanet_data.to_csv(r'C:\Users\LENOVO\OneDrive - Universidade de Aveiro\universidade\Mestrado\2º Ano\Tese\Códigos\Test_epanet\Samples\Richmond\48h_5min_richmond.csv',index=False)
...
#epanet_data = pd.read_csv(r'C:\Users\LENOVO\OneDrive - Universidade de Aveiro\universidade\Mestrado\2º Ano\Tese\Códigos\Test_epanet\Samples\Richmond\48h_manual_richmond.csv')
...
plt.suptitle('48h, 5 min Manual',fontsize=16, fontweight="bold")
plt.show()
...
plt.subplots_adjust(hspace=0.6)
plt.suptitle('48h Manual',fontsize=16, fontweight="bold")
plt.show()
