...
if _plat == 'Linux':
    try:
        _lib = ctypes.CDLL("./epanet/libepanet2.so_1.2")
    
    except:
        _lib = ctypes.CDLL(os.path.dirname(os.path.realpath(__file__)) + "/libepanet2.so_1.2")
elif _plat == 'Windows':
    try:
...
def ENsetlinkvalue(index, paramcode, value):
    ...
    paramcode: Link parameter codes consist of the following constants:
                 ...
                 * Use EN_INITSTATUS and EN_INITSETTING to set the design value for a link's status or setting that
                   exists prior to the start of a simulation. Use EN_STATUS and EN_SETTING to changes these values while
...
# it may changes in future versions
# ----------------------------------------------------------------------------------
...
EN_MIX1 = 0  # /* Tank mixing trained_models */
EN_MIX2 = 1
EN_FIFO = 2
EN_LIFO = 3
